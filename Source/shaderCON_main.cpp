...
//#include <FrameWork/SCONFW/Templates/sfw_grid_pipeline.h>
#include <FrameWork/SCONFW/Templates/sfw_triangles_pipeline.h>
 ...
 // to have icon images imported and displayed as part of the GUI !!!!!!!
    ...
    void run_loop() override {
       // buffer_data_panel.sconfw_pipeline = &sconfw_pipeline_test;
        ...
        Model mdl;
        std::string model_pathnem = "data/hex_surface.ply";
        ...
        printf("SCON :: number vertices %u : pipeline shader program id %i \n", unsigned int(sconfw_pipeline.vertex_shader.get_number_of_vertices()), sconfw_pipeline.output_shader_program.get_program());
...
// printf("MAIN 000 : %f : %f : %f\n", current_camera->position.x, current_camera->position.y, current_camera->position.z);
            ...
            // CRITICAL :: ImGui::Render() funtion must be placed here and NO ImGui code that is used 
            // can be below this function or the application will crash
            ImGui::Render();
...
//printf("Vertex %i : %i : %i : %i : %i\n", scon_vertex_data_panel.gl_in->size(), scon_vertex_data_panel.gl_out->size(), scon_vertex_data_panel.input_layouts->size(), scon_vertex_data_panel.output_layouts->size(), scon_vertex_data_panel.uniforms->size());
...
//printf("geometry %i : %i : %i : %i : %i\n", scon_geometry_data_panel.gl_in->size(), scon_geometry_data_panel.gl_out->size(), scon_geometry_data_panel.input_layouts->size(), scon_geometry_data_panel.output_layouts->size(), scon_geometry_data_panel.uniforms->size());
