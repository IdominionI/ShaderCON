...
public:
	...
	oglfw_camera_base_class *camera = nullptr;
...
//std::cout <<"camera_poperties_widget_class::display :111 : " << camera->id << "  :  "<< camera->getPosition().x << " : " << camera->getPosition().y << " : "<< camera->getPosition().z << std::endl;
		...
		if (ex_button("Orientate###cob", x_pos + 167, y_pos, 80, 20)) {
			camera->set_orientation( pitch, yaw, roll );
			camera->assign_last_mouse_translations();// Need this so does not revert to last location using mouse interaction
		}
...
//std::cout <<"camera_poperties_widget_class::display :3333 : " << camera->id << "  :  "<< camera->getPosition().x << " : " << camera->getPosition().y << " : "<< camera->getPosition().z << std::endl;
		...
		if (ex_button("Relativee###corb", x_pos + 167, y_pos, 80, 20)) {
			...
			camera->set_orientation( pitch + c_orientation.x, c_orientation.y + yaw,c_orientation.z + roll );
			camera->assign_last_mouse_translations();// Need this so does not revert to last location using mouse interaction
		}
		...
		float_input("###cmf", camera->movement_rate, x_pos + 225, y_pos, 50.0f);
		x_pos = 40; y_pos += 30;
		text("Mouse Rotation Sensitivity", x_pos, y_pos+3);
		float_input("###cms", camera->rotation_sensitivity, x_pos + 225, y_pos, 50.0f);// need to change this to a slider with min 0.05 to 10.0
		x_pos = 40; y_pos += 30;
		text("Mouse Scroll", x_pos, y_pos+3);
		float_input("###cmsc", camera->mouse_scroll_sensitivity, x_pos + 225, y_pos, 50.0f);// need to change this to a slider with min 0.05 to 10.0
		x_pos = 40; y_pos += 30;
		text("Key Rotation Sensitivity", x_pos, y_pos + 3);
		float_input("###cks", camera->key_rotation_factor, x_pos + 225, y_pos, 50.0f);// need to change this to a slider with min 0.05 to 10.0
		...
		if (ex_button("Look At###clab", x_pos + 120, y_pos, 75, 20)) {
			camera->look_at(camera->look_at_location);
		...
		if (ImGui::SliderFloat("###fov", &camera->fov, 1.0f, 175.0f, "%.2f")) {
			camera->far_clipping_plane = camera->fov;
		}
		...
		ImGui::SetCursorPosY(y_pos);
		ImGui::SetNextItemWidth(100);
		if (ImGui::SliderFloat("###ar", &camera->aspect_ratio, 1.0f, 20.0f, "%.2f")) {
		...
		ImGui::SetCursorPosY(y_pos);
		ImGui::SetNextItemWidth(100);
		if (ImGui::SliderFloat("###nc", &camera->near_clipping_plane, 0.0001f, 10000.0f, "%.2f")) {
			//camera->near_clipping_plane = camera->near_clipping_plane;
		}
		...
		ImGui::SetCursorPosY(y_pos);
		ImGui::SetNextItemWidth(100);
		if (ImGui::SliderFloat("###fc", &camera->far_clipping_plane, 1.0f, 100000.0f, "%.2f")) {
		...
		if (ImGui::Checkbox("Orthographic View ###cov", &otho_view)) {
			//camera_type_enum  camera_type = camera->get_camera_type();
			if (otho_view)
				camera->camera_type = camera_type_enum::ORTHOGRAPHIC;
			else
				camera->camera_type = camera_type_enum::PERSPECTIVE;
		}
