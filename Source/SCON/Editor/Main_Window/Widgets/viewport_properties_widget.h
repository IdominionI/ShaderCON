...
public:
	...
	oglfw_ref_grid_class* reference_grid = nullptr;
	...
	void display() {
		...
		if (reference_grid) {
			...
			if (ImGui::CollapsingHeader("Grid###vgridch")) {
				...
				y_pos += 25;
				text("Grid Origin Location :", x_pos, y_pos);
				y_pos += 25;
				ImGui::SetCursorPosX(x_pos + 70); ImGui::SetCursorPosY(y_pos); ImGui::SetNextItemWidth(75);
				ImGui::DragFloat("###vgriddcx", &reference_grid->grid_center_location.x, 0.005f, -FLT_MAX, +FLT_MAX, "%.3f", ImGuiSliderFlags_None);
				ImGui::SetCursorPosX(x_pos + 150); ImGui::SetCursorPosY(y_pos); ImGui::SetNextItemWidth(75);
				ImGui::DragFloat("###vgriddcy", &reference_grid->grid_center_location.y, 0.005f, -FLT_MAX, +FLT_MAX, "%.3f", ImGuiSliderFlags_None);
				ImGui::SetCursorPosX(x_pos + 230); ImGui::SetCursorPosY(y_pos); ImGui::SetNextItemWidth(75);
				ImGui::DragFloat("###vgriddcz", &reference_grid->grid_center_location.z, 0.005f, -FLT_MAX, +FLT_MAX, "%.3f", ImGuiSliderFlags_None);

				y_pos += 25;
				ImGui::SetCursorPosX(x_pos);
				ImGui::SetCursorPosY(y_pos);
				ImGui::Text("Grid Limit : ");
				ImGui::SameLine();
				ImGui::SetNextItemWidth(60);
				if (ImGui::InputInt("###gl", &reference_grid->grid_limit, 0)) {
					reference_grid->grid_limit = abs(reference_grid->grid_limit);
					reference_grid->initialise();
				}
				...
				*/
				...
				if (ImGui::BeginPopup("X-Y###plcpuxy")) {
					...
					reference_grid->xy_grid_color.r = xy_grid_color.x;
					reference_grid->xy_grid_color.g = xy_grid_color.y;
					reference_grid->xy_grid_color.b = xy_grid_color.z;
					reference_grid->xy_grid_color.a = xy_grid_color.w;
					ImGui::EndPopup();
				}
				...
				if (ImGui::BeginPopup("X-Z###plcpuxz")) {
					...
					reference_grid->xz_grid_color.r = xz_grid_color.x;
					reference_grid->xz_grid_color.g = xz_grid_color.y;
					reference_grid->xz_grid_color.b = xz_grid_color.z;
					reference_grid->xz_grid_color.a = xz_grid_color.w;
					ImGui::EndPopup();
				}
				...
				if (ImGui::BeginPopup("Y-Z###plcpuyz")) {
					...
					reference_grid->yz_grid_color.r = yz_grid_color.x;
					reference_grid->yz_grid_color.g = yz_grid_color.y;
					reference_grid->yz_grid_color.b = yz_grid_color.z;
					reference_grid->yz_grid_color.a = yz_grid_color.w;
					ImGui::EndPopup();
				}
		...
		if (display_crosshairs) {
			...
			if (ex_button("###plcxh", x_pos + 230, y_pos, 75, 20)) {
			...
			if (ImGui::BeginPopup("X-Hairs###plcpuxh")) {
				...
				//*xhair_color_viewport  = ImColor(ImVec4(xhair_color.x, xhair_color.y, xhair_color.z, xhair_color.w));
				*xhair_color_viewport = ImColor(xhair_color);
				ImGui::EndPopup();
			}
		}
		...
		if (background_color) {
			...
			ImGui::PushStyleColor(ImGuiCol_Button, { background_color->r,background_color->g,background_color->b,background_color->a });// Chanage all the buttons to the grid color until a ImGui::PopStyleColor
			if (ex_button("###plcvbg", x_pos + 190, y_pos, 75, 20)) {
				ImGui::OpenPopup("Background###plcpubg");
			}
			ImGui::PopStyleColor(1);// This must be present and the number represents the number of previous buttons that have been colored or will have a crash

			if (ImGui::BeginPopup("Background###plcpubg")) {
				ImGuiColorEditFlags flags = misc_flags;
				flags |= ImGuiColorEditFlags_NoAlpha; flags |= ImGuiColorEditFlags_NoSidePreview; flags |= ImGuiColorEditFlags_PickerHueWheel;
				ImGui::ColorPicker4("Background###cpbg", (float*)background_color, flags, ref_color ? &ref_color_v.x : NULL);
				ImGui::EndPopup();
			}
		}
	}
