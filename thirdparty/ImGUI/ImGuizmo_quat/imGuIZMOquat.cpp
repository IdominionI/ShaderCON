...
bool imguiGizmo::drawFunc(const char* label, float size)
{
    ...
    const float halfSquareSize = squareSize*0.5f;
...
#ifndef IMGUIZMO_USE_ONLY_ROT
    float screenFactor = innerSize.x / ((io.DisplaySize.x + io.DisplaySize.y) * 0.5f);
    ...
    if (ImGui::IsItemActive()) {
        ...
        //col.Value.w*=ImGui::GetStyle().Alpha; // Original line substituted with next line to have background controlled by main program !!!!!!!!
        col.Value.w = 0.0f;
        draw_list->AddRectFilled(controlPos, controlPos + innerSize, col, style.FrameRounding);
    }
    ...
    auto drawComponent = [&] (const int idx, const quat &q, ptrFunc func)
    {
        ...
        for(auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm+idx)->begin(); itVtx != ptrVtx->end(); ) { 
            ...
            for(int h=0; h<3; h++) {
                ...
                uv[h] = normalizeToControlSize(coord.x,coord.y);
                col[h] = addLightEffect(vec4(directionColor.x, directionColor.y, directionColor.z, 1.0f), norm.z, coord.z>0 ? coord.z : coord.z*0.5f);
            }
            addTriangle();
        }
    };
