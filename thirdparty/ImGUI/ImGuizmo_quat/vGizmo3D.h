...
private:
    ...
    tVec3 constPanDollyScale  = tVec3 (0.05f);  //pan scale
    const T constDistScale  = T( 0.01f);  //speed by distance sensibility

    T dollyScale = T(1.0f);  //dolly scale
    T panScale   = T(1.0f);  //pan scale
    T wheelScale = T(1.0f);  //dolly multiply for wheel step
    T distScale  = T(1.0f);  //speed by distance sensibility

    T povPanDollyFactor = T(0.0f); // internal use, maintain memory of current distance (pan/zoom speed by distance)
};
