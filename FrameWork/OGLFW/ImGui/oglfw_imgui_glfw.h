...
class oglFW_ImGui_GLFW_class {
public:
    oglFW_ImGui_GLFW_class() {}
    ~oglFW_ImGui_GLFW_class() {}
    ...
    bool ImGui_init(GLFWwindow* GFLW_window) {
        if (GFLW_window == NULL) {
            afw_globalc::get_current_logger()->log(LogLevel::CRITICAL, "Unable to initialize ImGui : Window not definde! ");
            return false;
        }
    ...
    void ImGui_pre_render() {
        ...
        // Uncomment if want docking within main window
    ...
    void ImGui_post_render(glfw_openGL_window_class *window) {
        // Rendering
        //ImGui::Render();

        ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData());

        //glfwSwapBuffers(window->get_window_ptr());// makes no difference
