...
#include "../Objects/oglfw_object_base.h"

#include "oglfw_scene_settings.h" // +++++
#include "oglfw_scene_cameras.h"
...
public:
	...
	std::vector<oglfw_object_base_class *> category_objects;

	oglfw_object_base_class *selected_object = NULL;

	oglfw_object_base_class *create_new_category_object( ) {
		oglfw_object_base_class *new_object = new oglfw_object_base_class;
		if (new_object == nullptr) return nullptr;
		new_object->object_category_id = objects_category_id;
		return new_object;
	}

	bool  add_object(oglfw_object_base_class* entity_object) {
		if (entity_object == nullptr) return false;
...
//std::cout << "vw_scene_objects_category_class:delete_object : AAAAA1111 : " << category_objects.size() << std::endl;
	...
	bool  delete_object(oglfw_object_base_class *entity_object) {
		if (entity_object == NULL) return false;
		return delete_object(entity_object->id);
	}

	oglfw_object_base_class* get_entity_object(id_type entity_id) {
		for (oglfw_object_base_class* object : category_objects) {
...
//printf("voxel_hcp_scene_objects_class :: get_object_index %i : %i\n", i, at(i)->object_id);
	...
	void render_objects(scene_settings_struct_type scene_settings,scene_cameras_manager_class scene_cameras) {
//	void render_objects() {
		oglfw_camera_base_class *selected_camera = scene_cameras.selected_camera;
...
//else std::cout <<"Scene_Object_Categories : Selected Camera != NULL\n";
		...
		for (oglfw_object_base_class *object : category_objects) {
			...
			if (object->display_object) {
				...
				GLint object_shader_id = object->geometry->shader->get_program();

				object->geometry->shader->use();// ++++++++
...
//std::cout <<"Scene_Object_Categories :!scene_lights.scene_global_light != NULL\n";
				...
				object->geometry->render_geometry();
			}
