...
#include "oglfw_scene_lights.h"

#include "oglfw_scene_settings.h" // +++++ place this include here above #include "scene_object_categories.h" and do not need to include in scene_object_categories.h
#include "oglfw_scene_object_categories.h"
...
//printf("scene_entities_db_manager2_class:define_new_entity_category : %i\n", scene_objects.size());
	...
	bool add_object(oglfw_object_base_class *entity_object, id_type entity_category_id) {
	...
	void display_entity_parameters(id_type current_selected_entity_type_id, id_type current_selected_entity_category_type_id, id_type current_selected_entity_id) {
		switch (current_selected_entity_type_id) {
			case ENTITY_TYPE_CAMERA : {
				oglfw_camera_base_class*camera = get_camera(current_selected_entity_id);
				if (camera != nullptr) {
					//scene_cameras.display_paramater_data_widget(camera);!!!!!!!!!!! YET TO BE IMPLEMENTED !!!!!!!!!!!!!!
				}
			} ;break;
...
//cout << "scene_entities_manager_class : display_entity_parameters : ENTITY_TYPE_OBJECT 00000 : " << current_selected_entity_id << " : " << current_selected_entity_category_type_id <<std::endl;
				oglfw_object_base_class *object = get_object_of_category(current_selected_entity_id, current_selected_entity_category_type_id);
				if (object != nullptr) {
//cout << "scene_entities_manager_class : display_entity_parameters : ENTITY_TYPE_OBJECT 11111 : object != nullptr" << std::endl;
					object->display_paramater_data_widget();//!!!!!!!!!!! YET TO BE IMPLEMENTED !!!!!!!!!!!!!!
				}
			}; break;
		}
	...
	oglfw_object_base_class *get_object_of_category(id_type entity_id, id_type entity_category_id) {
		if (entity_category_id > -1 && entity_id > -1) {
			int index = get_category_index(entity_category_id);
			if (index < 0) {
...
//std::cout << "scene_entities_manager_class :  get_objects_category_index : " << i << " : " << scene_objects[i].category_name << " : " << category_name << std::endl;
	...
	void render_overlays() {
		...
		//scene_lights.scene_lights_render();// !!!!!!!!!!! YET TO BE IMPLEMENTED !!!!!!!!!!!!!
		//scene_cameras.scene_cameras_render();// !!!!!!!!!!! YET TO BE IMPLEMENTED !!!!!!!!!!!!!
	}
