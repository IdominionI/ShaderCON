...
//#include <ofLight.h>
#include <FrameWork/OGLFW/Objects/oglfw_light.h>
...
#include <Framework/OGLFW/Application_OGLFW/oglfw_global.h> // Need to check that indeed unique ids are generated and stored

#include "../Widgets/oglfw_parameter_widget_base.h" 

#include <Framework/AFW/Tools/afw_id_key_manager.h>

inline id_key_manager_class<idu_type> light_id_key;// This needed as global so all catagories in all editors have a unique category id
...
public:
	...
	scene_lights_manager_class() {
		//selected_light = add_light(); // need at least one light to render images
		//selected_light->light_is_on = true;
	}
	...
	ofLight* add_light(bool new_light_selected = true) {
		...
		if (new_light != nullptr) {
			//new_light->id = afw_globalc::get_available_entity_id();
			new_light->id = light_id_key.get_available_id_key();
	...
	bool remove_light(id_type light_id) {
		...
		if (index > -1) {
			...
			//afw_globalc::assign_free_entity_id(light_id);// Free entity ID number to be reused when a new entity is created
			light_id_key.assign_free_id_key(light_id);// Free entity ID number to be reused when a new entity is created
			return true;
		}
		return false;
	}
	...
	parameter_widget_base_class   parameter_widget; // !!!!!!!!!!! ############## NEED TO DECOUPLE THIS FROM OGLFW ################# !!!!!!!!!!!!!
...
private:
	int get_light_index(id_type light_id) {
		for (size_t i = 0; i < lights.size(); i++) {
			if (lights[i]->id == light_id) return int(i);
		}
		return -1;
	}
