...
class oglfw_cube_class : public oglfw_entity_object_geometry_base_class {
public:
    oglfw_cube_class() { }
    ~oglfw_cube_class() { delete_geometry(), delete_buffers(); }
    ...
    // variables to be defined by external calls
    float angle = 0.0f;
    glm::vec3 location = { 0.0f, 1.0f, 0.0f };

    bool update_uniforms() override {

        auto cube_model = glm::mat4(1.0f);
        cube_model = glm::scale(cube_model, glm::vec3(1.0f, 1.0f, 1.0f));
        cube_model = glm::translate(cube_model, location);
        cube_model = glm::rotate(cube_model, angle, glm::vec3(1.0f, 1.0f, 1.0f));

        //shader.SetMat4("u_model", cube_model);
        shader->set_mat4(shader->get_program(), cube_model, "u_model");// +++++++++++++++
