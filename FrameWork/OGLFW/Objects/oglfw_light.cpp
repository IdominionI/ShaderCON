...
//static float_color globalAmbient(0.2, 0.2, 0.2, 1.0);
static float_color globalAmbient(0.2, 0.2, 0.2, 1.0);
/*
//----------------------------------------
...
//----------------------------------------
void ofSetGlobalAmbientColor(const float_color& c) {
	ofGetGLRenderer()->setGlobalAmbientColor(c);
	globalAmbient = c;
}
*/
const float_color& ofGetGlobalAmbientColor() {
	return globalAmbient;
}

//----------------------------------------
//std::vector<ofLight::Data *> &ofLightsData() {
//	static std::vector<ofLight::Data >* lightsActive = ofIsGLProgrammableRenderer() ? new vector<weak_ptr<ofLight::Data> > : new std::vector<weak_ptr<ofLight::Data> >(8);
//	return *lightsActive;
//}
...
ofLight::Data::Data() {
	glIndex = -1;
	isEnabled = false;
	attenuation_constant = 0.000001f;
	attenuation_linear = 0.000001f;
	attenuation_quadratic = 0.000001f;
	spotCutOff = 45.0f;
	exponent = 16.0f;
	width = 1.0f;
	height = 1.0f;
...
ofLight::ofLight()
	...
	setAmbientColor(afw_color.color(0, 0, 0));
	setDiffuseColor(afw_color.color(255, 255, 255));
	setSpecularColor(afw_color.color(255, 255, 255));
	setPointLight();
...
void ofLight::setSpotlightCutOff(float spotCutOff) {
	data->spotCutOff = glm::clamp(spotCutOff, 0.0f, 90.0f);
...
float ofLight::getSpotlightCutOff() const {
	if (!getIsSpotlight()) {
		...
		afw_globalc::get_current_logger()->log(LogLevel::ERROR, "oglfw_ight : getSpotlightCutOff(): light " + std::to_string(data->glIndex) + " is not a spot light");
	}
...
void ofLight::setSpotConcentration(float exponent) {
	data->exponent = glm::clamp(exponent, 0.0f, 128.0f);
...
float ofLight::getSpotConcentration() const {
	if (!getIsSpotlight()) {
		...
		afw_globalc::get_current_logger()->log(LogLevel::ERROR, "oglfw_ight : getSpotConcentration(): light " + std::to_string(data->glIndex) + " is not a spot light");
...
//----------------------------------------
void ofLight::setAmbientColor(const float_color& c) {
	data->ambientColor = c;
	//if (auto r = data->rendererP.lock()) {
...
//----------------------------------------
void ofLight::setDiffuseColor(const float_color& c) {
	data->diffuseColor = c;
...
//----------------------------------------
void ofLight::setSpecularColor(const float_color& c) {
...
//----------------------------------------
float_color ofLight::getAmbientColor() const {
	return data->ambientColor;
}

//----------------------------------------
float_color ofLight::getDiffuseColor() const {
	return data->diffuseColor;
}

//----------------------------------------
float_color ofLight::getSpecularColor() const {
