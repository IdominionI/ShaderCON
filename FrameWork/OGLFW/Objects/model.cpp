...
//	namespace eng {
		...
		void Model::Mesh::Draw(bool instanced, int iCount)
		{
			...
			if (instanced)
				glDrawElementsInstanced(GL_TRIANGLES, GLsizei(Indices.size()), GL_UNSIGNED_INT, nullptr, iCount);
			else
		...
		bool Model::LoadFromFile(const std::string& path)
		{
			...
			afw_globalc::get_current_logger()->log(LogLevel::INFO, "Loading a 3D model from file \"" + path + "\"");
			...
			if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) // if is Not Zero
			{
				...
				afw_globalc::get_current_logger()->log(LogLevel::INFO, "Assimp has detected an error \"" + std::string(importer.GetErrorString()) + "\"");
				return false;
			}
