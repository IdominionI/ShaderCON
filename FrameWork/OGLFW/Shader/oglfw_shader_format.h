...
#include <iostream>// testing only : comment out when not using std::cout

//#define GLEW_STATIC
//#include <gl/glew.h>
#include <glm/glm.hpp>

#include "oglfw_shader_definitions.h"

#include <Framework/AFW/Tools/afw_file_tools.h>

#include "oglfw_shader_parameters.h"
...
struct shader_format_class {
	...
	unsigned int shader_program_id = 0;				   // ID number of shader program witrh the
...
//		switch (shader_type) {
//			case shader_type_enum::vertex:   full_pathname = glsl_vertex_shader_file_pathname.string();      break;
//			case shader_type_enum::geometry: full_pathname = glsl_geometry_shader_file_pathname.string();    break;
//			case shader_type_enum::fragment: full_pathname = glsl_fragment_shader_file_pathname.string();    break;
////case shader_type_enum::tesselation : full_pathname = std::filesystem::absolute(glsl_tesselation_shader_file_pathname).string(); break;
//			case shader_type_enum::compute:  full_pathname = compute_shader_file_pathname.string();          break;
//		}

//std::cout << "define_shader_to_compile 000 " << shader_source_pathname << " : " << full_pathname << std::endl;

		std::string source_code = FW::filetools::read_all(full_pathname);
//std::cout << "define_shader_to_compile 111 \n" << full_pathname << "|\n<";
//std::cout << "define_shader_to_compile 111 \n" << full_pathname << "|\n<" << source_code << ">\n";
		if (source_code.size() < 1) return false;
//std::cout << "define_shader_to_compile 222 \n" ;
		define_shader_source_code(shader_type, source_code);
//std::cout << "define_shader_to_compile 333 \n";
		return true;
	}
