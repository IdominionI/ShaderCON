...
//typedef afw_geometry_base_class<VT>

template <class VT>
class afw_entity_object_geometry_base_class : public afw_geometry_base_class<VT> {
public:
	using GBC = afw_geometry_base_class<VT>;
	using VBC = afw_vertices_base_class<VT>;

	// Usage : GBC::afw_geometry_base_class variable function etc
	// Usage : VBC::afw_vertices_base_class variable function etc

	afw_entity_object_geometry_base_class() {
		//entity_base_geometry_class(ofNode *node = nullptr) {
		GBC geometry_type = geometry_type_enum::undefined;
		//parent_node = node;
	}
	virtual ~afw_entity_object_geometry_base_class() {
		//delete_shader();
	}

	int geometry_object_id = INVALID_ID;

	draw_mode_enum geometry_draw_mode = draw_mode_enum::UFW_UNDEFINED;

	//ofNode* node = nullptr; !!!! ******* THIS NEEDS TO BE ATTENDED TO !!!!!!!!!!
	...
	virtual bool init() {
		...
		set_geometry_draw_mode(draw_mode_enum::UFW_UNDEFINED);//
		GBC::geometry_type = geometry_type_enum::undefined;

		//VBC::vertices_changed = false;
		//if (VBC::have_vertices_changed()) {
	...
	virtual bool load_geometry(const std::string& filepath, float x_off) { return false; }
	virtual bool save_geometry(const std::string& filepath) { return false; }
	...
	//}
	/*
	virtual void draw_normals(float length, bool bFaceNormals, glm::vec4 color_normals) const {
...
/*
	bool define_geometry_attributes_shader(oglfw_shader_class* _shader = NULL) {
		if (_shader == NULL) {
			if (shader == NULL)
				return false;
		}
