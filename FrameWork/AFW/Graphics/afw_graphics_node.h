...
class geometry_node_base_class;

typedef std::pair<geometry_node_base_class *, int> node_association_data_type;

class afw_graphics_node_associations_class {
public:
	~afw_graphics_node_associations_class() {
		clear_node_associations();
	}

	//struct node_associations_struct_type {
	//	geometry_node_base_class *node;
	//	int                      association_type;
	//};

	std::vector<node_association_data_type> node_associations;

	bool  add_node_association(geometry_node_base_class *entity, int association_type) {
		if (entity == NULL) return false;

		node_association_data_type node_association;

		node_association.first  = entity;
		node_association.second = association_type;
	...
	bool remove_all_node_associations_for_node(geometry_node_base_class *node) {
		if (node == NULL) return false;
		for (size_t i = node_associations.size() - 1; i >= 0; i--) {
			if (node_associations[i].first == node) {
...
class afw_graphics_node_base_class {
public:
	afw_graphics_node_base_class();
	virtual ~afw_graphics_node_base_class() {};
...
//*
	...
	///
	/// \param parent Reference to the afw_graphics_node_base_class which becomes the parent node.
	/// \param bMaintainGlobalTransform Boolean if maintain child's global
	///        transformations (default = false). 
	void setParent(afw_graphics_node_base_class& parent, bool bMaintainGlobalTransform = false);
	...
	///
	/// \returns Pointer to parent afw_graphics_node_base_class.
	afw_graphics_node_base_class* getParent() const;
	...
	/// \note This version calculates the up vector automatically to try to keep
	///       it relatively consistant with the original angle.
	void lookAt(const afw_graphics_node_base_class& lookAtNode);

	/// \brief Orient node to look at another node (-z axis pointing at other node).
	/// \param lookAtNode A reference to the node to look at.
	/// \param upVector The desired up axis as a ref to cartesian 3D vector.
	void lookAt(const afw_graphics_node_base_class& lookAtNode, const glm::vec3& upVector);
	...
	//OF_DEPRECATED_MSG("Use the Deg/Rad version.", void orbit(float longitude, float latitude, float radius, afw_graphics_node_base_class& centerNode));
	...
	///        Default = (0, 0, 0).	
	void orbitDeg(float longitude, float latitude, float radius, afw_graphics_node_base_class& centerNode);
	...
	///        Default = (0, 0, 0).	
	void orbitRad(float longitude, float latitude, float radius, afw_graphics_node_base_class& centerNode);
	...
	/// call afw_graphics_node_base_class::transform(); write your draw code, and afw_graphics_node_base_class::restoreTransform();
	/// OR A simpler way is to extend afw_graphics_node_base_class and override afw_graphics_node_base_class::customDraw();
	...
	/// \brief If you extend afw_graphics_node_base_class and wish to change the way it draws, extend this.
...
protected:
	...
	/// \brief Classes extending afw_graphics_node_base_class can override this method to get
	///        notified when the position changed.
	virtual void onPositionChanged() {}

	/// \brief Classes extending afw_graphics_node_base_class can override this methods to get notified
	...
	afw_graphics_node_base_class* parent = nullptr;
...
private:
	...
	//std::set<afw_graphics_node_base_class*> children;

	//void addListener(afw_graphics_node_base_class& node);
	//void removeListener(afw_graphics_node_base_class& node);
	...
	glm::vec3 geometry_position = glm::vec3{ 0.0,0.0,0.0 };
	glm::quat geometry_orientation = glm::quat{ 0.0,0.0,0.0,0.0 };
	glm::vec3 geometry_scale = glm::vec3{ 1.0,1.0,1.0 };
