...
//#include <FrameWork/AFW/Application/ofAppRunner.h>
//#include <FrameWork/AFW/Events/afw_event.h>
...
uint64_t ofGetFixedStepForFps(double fps) {
	return uint64_t(1000000000 / fps);
}
...
//	if (!mainLoop) {
//		std::cout <<"ofSetSystemTimeMode" << "ofMainLoop is not initialized yet, can't set time mode" << std::endl;
//		return;
//	}
//	auto window = mainLoop->getCurrentWindow();
//	if (!window) {
//		std::cout <<"ofSetSystemTimeMode" << "No window setup yet can't set time mode" << std::endl;
//		return;
//	}
...
float ofGetElapsedTimef() {
	...
	return std::chrono::duration<float>(of::priv::getClock().getElapsedTime()).count();
}
...
static std::locale getLocale(const string& locale) {
	...
	if (printonce) {
		...
		setlocale(LC_ALL, "");
		std::cout << "ofUtils" << "std::locale not supported. Using C locale: " << current << std::endl;
		printonce = false;
	}
	//#else
		//try {
		...
		//}
		//catch (...) {
		//	std::cout << "ofUtils" << "Couldn't create locale " << locale << " using default, " << loc.name() << std::endl;
		//}
	//#endif
	return loc;
}
...
void ofLaunchBrowser(const string& url, bool uriEncodeQuery) {
	...
	if (uriParseUriA(&state, url.c_str()) != URI_SUCCESS) {
		std::cout <<"ofUtils") << "ofLaunchBrowser(): malformed url \"" << url << "\"";
	...
	if (scheme != "http" && scheme != "https") {
		std::cout <<"ofUtils") << "ofLaunchBrowser(): url does not begin with http:// or https://: \"" << uriStr << "\"";
		return;
	}
	...
	if (ret != 0) {
		std::cout <<"ofUtils") << "ofLaunchBrowser(): couldn't open browser, commandStr \"" << commandStr << "\"";
	}
#endif
	...
	std::cout <<"ofUtils") << "ofLaunchBrowser() not implementeed in emscripten";
#endif
}
...
string ofSystem(const string& command) {
	...
	if (ret == nullptr) {
		std::cout <<"ofUtils") << "ofSystem(): error opening return file for command \"" << command << "\"";
	}
	else {
		c = fgetc(ret);
		while (c != EOF) {
