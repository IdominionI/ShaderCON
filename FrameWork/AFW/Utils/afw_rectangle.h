...
///
/// afw_rectangle_class is a simple container for describing the position and size of a
/// 2D rectangle. The afw_rectangle_class::standardize() method can be used to ensure
/// that the origin is in the "standard" form.
///
/// \warning While afw_rectangle_class takes glm::vec3
/// all afw_rectangle_class operations are 2D only, ignoring the z-component.
class afw_rectangle_class {
public:
    ...
    afw_rectangle_class(float px, float py, float w, float h);
    ...
    /// \param w The width of the rectangle.
    /// \param h The height of the rectangle.
    afw_rectangle_class(const glm::vec3& p, float w, float h);
    afw_rectangle_class(const glm::vec2& p, float w, float h);

    /// \brief Construct a rectangle by copying another rectangle.
    /// \param rect The rectangle to copy.
    afw_rectangle_class(const afw_rectangle_class& rect);
    ...
    afw_rectangle_class(const glm::vec3& p0, const glm::vec3& p1);
    afw_rectangle_class(const glm::vec2& p0, const glm::vec2& p1);

    /// \brief Destroy the rectangle.
    virtual ~afw_rectangle_class();
    ...
    /// \param rect The rectangle to copy.
    void set(const afw_rectangle_class& rect);
    ...
    /// \brief Scales both the width and height the afw_rectangle_class from its center.
    ///
    ...
    /// \brief Scale the rectanle using a target afw_rectangle_class and ofScaleMode.
    ///
    /// Adjusts the afw_rectangle_class to match the provided afw_rectangle_class using the
    /// provided ofScaleMode.
    ///
    /// \param targetRect The Target Rectangle to scale to.
    /// \param scaleMode The scale mode to use when scaling.
    void scaleTo(const afw_rectangle_class& targetRect,
        ofScaleMode scaleMode = OF_SCALEMODE_FIT);


    /// \brief Scale the rectangle using a target afw_rectangle_class and parameters.
    ///
    /// Scales the afw_rectangle_class to match the target afw_rectangle_class. It will use the
    /// ofAspectRatioMode to scale the afw_rectangle_class and will use the
    ...
    /// \brief Scale the rectangle using a target afw_rectangle_class and parameters.
    ///
    /// Adjusts the afw_rectangle_class to match the passed-in afw_rectangle_class. It will
    /// use the ofAspectRatioMode to scale the afw_rectangle_class, and will use the
    ...
    ///
    /// Aligns the horizontal position of the afw_rectangle_class to the given x-
    ...
    ///
    /// Aligns the horizontal position of the afw_rectangle_class to that of the
    /// passed-in afw_rectangle_class using an ofAlignHorz constant as anchor.
    ///
    ...
    void alignToHorz(const afw_rectangle_class& targetRect,
        ofAlignHorz sharedAnchor = OF_ALIGN_HORZ_CENTER);

    /// \brief Horizontally align two rectangles to another using two anchors.
    ///
    /// Aligns the horizontal position of the afw_rectangle_class to that of the
    /// passed-in afw_rectangle_class. Takes two ofAlignHorz constants as anchors.
    ///
    /// Will align the chosen anchor in the afw_rectangle_class with the chosen anchor
    /// in the passed-in afw_rectangle_class.
    ///
    /// \param targetRect The target rectangle to align this rectangle to.
    ...
    ///
    /// Aligns the vertical position of the afw_rectangle_class to the given y-
    ...
    /// \param sharedAnchor The alignment position of this afw_rectangle_class to use.
    ...
    ///
    /// Aligns the vertical position of the afw_rectangle_class to that of the
    /// passed-in afw_rectangle_class using an ofAlignVert constant as anchor.
    ///
    ...
    ///
    /// \param targetRect The target afw_rectangle_class to align this rectangle to.
    /// \param sharedAnchor The alignment position for both rectangles to use.
    void alignToVert(const afw_rectangle_class& targetRect,
        ofAlignVert sharedAnchor = OF_ALIGN_VERT_CENTER);
    ...
    /// \brief Align this afw_rectangle_class to an glm::vec3 in both x- and y dimentions.
    ///
    /// Aligns the position of the afw_rectangle_class to the given point using an
    ...
    /// \brief Align this afw_rectangle_class to an glm::vec3 in both x- and y dimentions using a shared anchor.
    ///
    /// Aligns the position of the afw_rectangle_class to that of the passed-in
    /// afw_rectangle_class. Can take an ofAlignHorz constant and an ofAlignVert
    /// constant.
    ///
    ...
    void alignTo(const afw_rectangle_class& targetRect,
        ofAlignHorz sharedHorzAnchor = OF_ALIGN_HORZ_CENTER,
        ofAlignVert sharedVertAnchor = OF_ALIGN_VERT_CENTER);


    /// \brief Vertically align this afw_rectangle_class to another target afw_rectangle_class.
    ///
    /// Aligns the position of the afw_rectangle_class to that of the passed-in
    /// afw_rectangle_class. Takes two ofAlignHorz constants and two ofAlignVert
    /// constants.
    ///
    /// Will align the chosen anchors in the afw_rectangle_class with the chosen
    /// anchors in the passed-in afw_rectangle_class.
    ///
    /// \param targetRect The target afw_rectangle_class to align this rectangle to.
    ...
    /// \brief Determines if the coordinates (x, y) are within the afw_rectangle_class.
    ///
    /// Note that coordinates on the edge of the afw_rectangle_class are not
    /// considered within the rectangle and will return false.
    ///
    /// \param px The x-coordinate to test.
    /// \param py The y-coordinate to test.
    /// \returns true if px and py are inside this afw_rectangle_class.
    bool inside(float px, float py) const;

    /// \brief Determines if the glm::vec3 is within the afw_rectangle_class.
    ///
    /// Note that points on the edge of the afw_rectangle_class are not
    /// considered within the rectangle and will return false.
    ///
    /// \warning The z-component of the passed glm::vec3 is ignored.
    ///
    /// \param p The point to test.
    /// \returns true if the point p is inside this afw_rectangle_class.
    bool inside(const glm::vec3& p) const;
    bool inside(const glm::vec2& p) const;

    /// \brief Determines if another afw_rectangle_class is completely within the afw_rectangle_class.
    ///
    /// Note that rectangles that share an edge with the afw_rectangle_class are not
    /// considered within the rectangle and will return false.
    ///
    /// \param rect The afw_rectangle_class to test.
    /// \returns true if all four corners of the rect are within this rectangle.
    bool inside(const afw_rectangle_class& rect) const;

    /// \brief Determines if both of the passed glm::vec3s are within the afw_rectangle_class or not.
    ///
    /// Note that points on the edge of the afw_rectangle_class are not considered
    ...
    /// \returns `true` if the area contained within the afw_rectangle_class overlaps
    ///     with the area contained within the passed afw_rectangle_class.
    bool intersects(const afw_rectangle_class& rect) const;

    /// \brief Determines if a line segment intersects with the afw_rectangle_class.
    ///
    /// \warning The z-components of the passed glm::vec3s are ignored.
    ///
    /// \param p0 The first point to test.
    ...
    ///     either crosses the perimeter of the afw_rectangle_class or is completely
    ///     contained within.
    //bool intersects(const glm::vec3& p0, const glm::vec3& p1) const;
    //bool intersects(const glm::vec2& p0, const glm::vec2& p1) const;

    /// \brief Grow the afw_rectangle_class to include the given (x, y) coordinates.
    ///
    /// This will potentially change the width, height, x-position, and y-
    /// position of the afw_rectangle_class. If the points are already within the
    ...
    /// \brief Grow the afw_rectangle_class to include the given point.
    ///
    /// This will potentially change the width, height, x-position, and y-
    /// position of the afw_rectangle_class.  If the point is already within the
    /// rectangle, this rectangle will remain unchanged.
    ///
    /// \warning The z-component of the passed glm::vec3 is ignored.
    ///
    ...
    /// \brief Grow the afw_rectangle_class to include the given afw_rectangle_class.
    ///
    /// This will potentially change the width, height, x-position, and y-
    /// position of the afw_rectangle_class.  If the rectangle is already within the
    /// rectangle, this rectangle will remain unchanged.
    ///
    /// \param rect The rectangle to include.
    void growToInclude(const afw_rectangle_class& rect);

    /// \brief Grow the afw_rectangle_class to include the given line segment.
    ///
    /// This will potentially change the width, height, x-position, and
    /// y-position of the afw_rectangle_class.  If the points are already within the
    /// rectangle, this rectangle will remain unchanged.
    ///
    /// \warning The z-components of the passed glm::vec3s are ignored.
    ///
    ...
    /// \returns A new afw_rectangle_class representing the intersecting area or an
    ///          empty rectangle (0, 0, 0, 0) if there is no intersection.
    afw_rectangle_class getIntersection(const afw_rectangle_class& rect) const;

    /// \brief Get the union area between this rectangle and anohter.
    ///
    /// \sa getIntersection(const afw_rectangle_class& rect) const
    /// \param rect The rectangle to unite with.
    /// \returns A new afw_rectangle_class whose area contains both the area of the
    ///          this rectangle and the passed rectangle..
    afw_rectangle_class getUnion(const afw_rectangle_class& rect) const;
    ...
    ///
    /// \returns the Standardized version of this afw_rectangle_class.
    afw_rectangle_class getStandardized() const;

    /// \brief Returns true if this afw_rectangle_class is standardized.
    ///
    /// "Standardized" rectangles are rectangles whose width and height are positive. 
    /// For more information about standardized rectangles, see the discussion of the standardize() method.
    ///
    ...
    /// \brief Get the area of the afw_rectangle_class.
    ///
    ...
    /// \brief Gets the perimeter of the afw_rectangle_class.
    ///
    ...
    /// \brief Gets the ratio of width to height of the afw_rectangle_class.
    ///
    /// \returns The aspect ratio of the rectangle.
    float getAspectRatio() const;

    /// \brief Determines if the afw_rectangle_class's area is zero.
    /// \sa getArea().
    /// \returns true if both the width == 0 and height == 0, false if either is non-zero.
    bool isEmpty() const;

    /// \brief Get the minimum x and y coordinates of the afw_rectangle_class as glm::vec3.
    /// \returns The minimum x and y coordinates of the rectangle.
    glm::vec3 getMin() const;

    /// \brief Get the maximum x and y coordinates of the afw_rectangle_class as glm::vec3.
    /// \returns The maximum x and y coordinates of the rectangle.
    glm::vec3 getMax() const;

    /// \brief Get the smallest x position of the afw_rectangle_class as float.
    /// \returns The smallest xposition of the rectangle.
    float getMinX() const;

    /// \brief Get the largest x position of the afw_rectangle_class as float.
    /// \returns The largest x position of the rectangle.
    float getMaxX() const;

    /// \brief Get the smallest y position of the afw_rectangle_class as float.
    /// \returns The smallest y position of the rectangle.
    float getMinY() const;

    /// \brief Get the largest y position of the afw_rectangle_class as float.
    /// \returns The largest y position of the rectangle.
    float getMaxY() const;

    /// \brief Gets the x position of the left edge of the afw_rectangle_class as float.
    /// \sa getMinX()
    /// \returns The x position of the left edge of the rectangle.
    float getLeft() const;

    /// \brief Gets the x position of the right edge of the afw_rectangle_class as float.
    /// \sa getMaxX()
    /// \returns The y position of the right edge of the rectangle.
    float getRight() const;

    /// \brief Gets the y position of the top edge of the afw_rectangle_class as float.
    /// \sa getMinY()
    /// \returns The y position of the top edge of the rectangle.
    float getTop() const;

    /// \brief Gets the y position of the bottom edge of the afw_rectangle_class as float.
    /// \sa getMaxY()
    /// \returns The y position of the bottom edge of the rectangle.
    float getBottom() const;

    /// \brief Get the top-left coordinates of the afw_rectangle_class as glm::vec3.
    /// \sa getMin()
    /// \returns The top-left coordinates of the rectangle.
    glm::vec3 getTopLeft() const;

    /// \brief Get the top-right coordinates of the afw_rectangle_class as glm::vec3.
    /// \returns The top-right coordinates of the rectangle.
    glm::vec3 getTopRight() const;

    /// \brief Get the bottom-left coordinates of the afw_rectangle_class as glm::vec3.
    /// \returns The bottom-left coordinates of the rectangle.
    glm::vec3 getBottomLeft() const;

    /// \brief Get the bottom-right coordinates of the afw_rectangle_class as glm::vec3.
    /// \sa getMax()
    ...
    /// the horizontal edges of the afw_rectangle_class using the `ofAlignHorz` enum.
    ///
    ...
    /// vertical edges of the afw_rectangle_class using the ofAlignVert enum.
    ///
    /// \param anchor The anchor position to query.
    /// \returns the value of the referenced anchor position.
    float getVertAnchor(ofAlignVert anchor) const;

    /// \brief Get the afw_rectangle_class's position.
    ///
    ...
    /// \brief Get the coordiantes of the afw_rectangle_class's center as glm::vec3.
    ///
    ...
    /// \brief Gets the x position of the afw_rectangle_class as float.
    /// \returns The x position of the rectangle.
    float getX() const;

    /// \brief Gets the y position of the afw_rectangle_class as float.
    /// \returns the y position of the rectangle.
    float getY() const;

    /// \brief Gets the width of the afw_rectangle_class as float.
    /// \returns The width of the rectangle.
    float getWidth() const;

    /// \brief Gets the height of the afw_rectangle_class as float.
    ...
    /// \returns The mapped coordinate
    //afw_rectangle_class map(const afw_rectangle_class& coeff) const;
    ...
    //afw_rectangle_class mapClamp(const afw_rectangle_class& coeff) const;
    ...
    afw_rectangle_class& operator = (const afw_rectangle_class& rect);

    /// \brief Returns a new afw_rectangle_class where the x and y positions of the
    /// rectangle are offset by the (x, y) coordinates of the glm::vec3.
    /// \param p The point to translate.
    /// \returns The translated afw_rectangle_class.
    afw_rectangle_class operator + (const glm::vec3& p);
    afw_rectangle_class operator + (const glm::vec2& p);
    ...
    /// \brief If both afw_rectangle_classs have the same x, y, width, and height,
    /// they are considered equal.
    /// \param rect The rectangle to compare.
    /// \returns True if the rectangles are equal.
    //bool operator == (const afw_rectangle_class& rect) const;

    /// \brief If the two afw_rectangle_classs differ in x, y, width, or height, they
    ...
    /// \brief The (x,y) position of the afw_rectangle_class as an glm::vec3.
    ///
    /// \warning The z-component of this position is preserved and can be used
    /// but all afw_rectangle_class operations will ignore the z-component.
    glm::vec3 position{};

    /// \brief The x position of the afw_rectangle_class.
    float& x;

    /// \brief The y position of the afw_rectangle_class.
    float& y;

    /// \brief The width of the afw_rectangle_class.
    float width;

    /// \brief The height of the afw_rectangle_class.
    float height;
...
/// is not used.
//std::ostream& operator<<(std::ostream& os, const afw_rectangle_class& rect);
/// \warning The internal z component of the glm::vec3 is preserved even though it
/// is not used.
//std::istream& operator>>(std::istream& is, afw_rectangle_class& rect);
/// \endcond
