...
//	std::filesystem::path defaultDataPath(){
	std::string defaultDataPath() {
		...
		try {
			return std::filesystem::canonical(ofFilePath::getCurrentExeDir() / std::filesystem::path("../../../data/")).string();
		}
		catch (...) {
			return (ofFilePath::getCurrentExeDir() / std::filesystem::path("../../../data/")).string();
		}
...
//------------------------------------------------------------------------------------------------------------
//	MARK: - near future
//std::filesystem::path ofFile::path() const {
//return myFile;
...
//------------------------------------------------------------------------------------------------------------
//	MARK: - near future
//std::filesystem::path ofFile::getEnclosingDirectory() const {
...
//------------------------------------------------------------------------------------------------------------
//	MARK: - near future
//std::filesystem::path ofFile::getAbsolutePath() const {
...
//#ifdef TARGET_WIN32
	...
	DWORD attr = GetFileAttributesW(myFile.native().c_str()); // found this in an internet search : need to change
...
//#else
//	if (geteuid() == info.st_uid) {
//		return perm & std::filesystem::perms::owner_read;
//	}
//	else if (getegid() == info.st_gid) {
//		return perm & std::filesystem::perms::group_read;
//	}
//	else {
//		return perm & std::filesystem::perms::others_read;
//	}
//#endif
//#endif
}
...
//------------------------------------------------------------------------------------------------------------
//	MARK: - near future
//std::filesystem::path ofDirectory::path() const {
//	return myDir;
...
//------------------------------------------------------------------------------------------------------------
//	MARK: - near future
//std::filesystem::path ofDirectory::getAbsolutePath() const {
//	try{
//		return std::filesystem::canonical(std::filesystem::absolute(myDir));
//	}catch(...){
//		return std::filesystem::absolute(myDir);
//	}
//}
...
//------------------------------------------------------------------------------------------------------------
std::size_t ofDirectory::listDir(const std::filesystem::path& directory) {
	open(directory);
	return listDir();
}
...
void ofDirectory::sort(const SortMode& mode) {
	...
	if (mode == ofDirectory::SORT_NATURAL) {
		...
		files.reserve(sort.size());
		for (auto& s : sort) {
			files.emplace_back(myDir / std::filesystem::path(s), ofFile::Reference);
		}
	}
...
//------------------------------------------------------------------------------------------------------------
//	MARK: - near future
//std::filesystem::path ofFilePath::addTrailingSlash(const std::filesystem::path & _path){
std::string ofFilePath::addTrailingSlash(const std::filesystem::path& _path) {
	...
	// return (std::filesystem::path(_path).make_preferred() / "");
...
// MARK: - near future
// std::filesystem::path ofFilePath::removeExt(const std::filesystem::path& _filename){
std::string ofFilePath::removeExt(const std::filesystem::path& _filename) {
...
//------------------------------------------------------------------------------------------------------------
// FIXME: convert to std::filesystem::path
...
//------------------------------------------------------------------------------------------------------------
// FIXME: is this still useful? if yes convert to std::filesystem::path
...
//------------------------------------------------------------------------------------------------------------
//	MARK: - near future
//std::filesystem::path ofFilePath::getEnclosingDirectory(const std::filesystem::path & _filePath, bool bRelativeToData){
...
//------------------------------------------------------------------------------------------------------------
// FIXME: - near future
//std::filesystem::path ofFilePath::getAbsolutePath(const std::filesystem::path& path, bool bRelativeToData){
...
//------------------------------------------------------------------------------------------------------------
// MARK: - near future
//std::filesystem::path ofFilePath::join(const std::filesystem::path& path1, const std::filesystem::path& path2){
...
// MARK: - near future
//std::filesystem::path ofFilePath::makeRelative(const std::filesystem::path & from, const std::filesystem::path & to){
...
std::string ofToDataPath(const std::filesystem::path& path, bool makeAbsolute) {
	...
	auto relativeDirDataPath = ofFilePath::makeRelative(std::filesystem::current_path(), dataPath);
	...
	if (makeAbsolute) {
		...
		try {
			...
			// return std::filesystem::absolute(outputPath);
		}
	}
	else {
