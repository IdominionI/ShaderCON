...
//Clone of BabylonCpp string_tools.h with aim to incorporate into a FWstring class
...
namespace FW {
    namespace stringtools {
        ...
        inline std::string stripComments(const std::string& input, const std::string& delimiters = "//")
        {
            ...
            int delimitersIndex = FW::stringtools::indexOf(input, delimiters);
        ...
        inline std::string truncate(const std::string& str, size_t index) {
            if (index < 1 || index >= str.length()) return str;
