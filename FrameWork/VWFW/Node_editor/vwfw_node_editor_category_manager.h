...
#include "vwfw_node_editor_basis.h"

class vwfw_nodes_of_category_class {
public:
	~vwfw_nodes_of_category_class() {
		clear_all_editor_nodes();
	}

	id_type editor_node_category_id;

	std::vector<vwfw_editor_node_basis*> editor_category_nodes;

	//bool add_editor_node(vwfw_editor_node_basis* editor_node, scene_manager_class* scene_manager, log_panel_class* log_panel) {
	bool add_editor_node(vwfw_editor_node_basis* editor_node, scene_entities_manager_class* scene_manager, log_panel_class* log_panel) {
	...
	void clear_all_editor_nodes() {
		for (vwfw_editor_node_basis *editor_category_node : editor_category_nodes) {
	...
	bool perform_editor_node_creation(node_type_struct editor_node_definition, graph_class *graph, scene_entities_manager_class* scene_manager, log_panel_class* log_panel, ImVec2 click_pos) {
		int index = find_editor_node_creation_function(editor_node_definition);
//printf("vwfw_nodes_of_category_class :perform_editor_node_creation %i : %i \n ",int(editor_category_nodes.size()), index);
		if (index < 0) {
...
public:
	std::vector<vwfw_nodes_of_category_class*> nodes_of_category;
	...
	int add_editor_node_category(id_type editor_nodes_category_id, log_panel_class* log_panel) {
		...
		vwfw_nodes_of_category_class* new_editor_category_nodes = new vwfw_nodes_of_category_class;
	...
	void clear_all_editor_node_creation_functions() {
		for (vwfw_nodes_of_category_class* node_of_category : nodes_of_category) {
